...
# Localiza arquivo archive (17).zip ou archive(17).zip
for f in os.listdir(desktop):
    if "archive" in f.lower() and "17" in f and f.endswith(".zip"):
        target_zip = os.path.join(desktop, f)
        break

if not target_zip:
    print("Arquivo archive(17).zip não encontrado. Lista de zips no Desktop:")
    for f in os.listdir(desktop):
        if f.endswith(".zip"):
            print(" ", f)
    exit(1)
...
dest_copy = os.path.join(scratch_dir, "archive(17).zip")
...
with zipfile.ZipFile(dest_copy, "r") as z:
    for item in z.infolist():
        ...
        if target_name == "README.md":
            target_name = "README_sp500_nasdaq_5m.md"
    ...
    if name.endswith(".csv"):
        ...
        with open(csv_path, "r", encoding="utf-8", errors="ignore") as f:
            ...
            for r in reader:
                ...
                last_row = r
        print(f"\nAmostra de {name}:")
        print(f"   Total de velas M5: {count:,}")
