...
# Localiza arquivo com 11
for f in os.listdir(desktop):
    if "archive" in f.lower() and "11" in f and f.endswith(".zip"):
        target_zip = os.path.join(desktop, f)
        break

if not target_zip:
    print("Arquivo archive(11).zip não encontrado. Lista de zips no Desktop:")
    for f in os.listdir(desktop):
        if f.endswith(".zip"):
            print(" ", f)
    exit(1)

print(f"1. Arquivo localizado: {target_zip} ({os.path.getsize(target_zip) / (1024*1024):.2f} MB)")

scratch_dir = r"C:\Users\rafae\.gemini\antigravity\brain\c125241d-a48c-4ba7-aab4-bba800620ab2\scratch"
os.makedirs(scratch_dir, exist_ok=True)
dest_copy = os.path.join(scratch_dir, "archive(11).zip")
...
print(f"3. Inspecionando e extraindo na pasta {poly_dir}...")
...
with zipfile.ZipFile(dest_copy, "r") as z:
    for item in z.infolist():
        ...
        if target_name == "README.md":
            target_name = "README_sp500_5years.md"
...
for name, size in extracted_files[:10]:
    print(f"   - {name} ({size / (1024*1024):.2f} MB)")

# Se for um único CSV mestre ou pasta de CSVs
main_csv = None
...
if main_csv and os.path.exists(main_csv):
    with open(main_csv, "r", encoding="utf-8", errors="ignore") as f:
        ...
        for r in reader:
            count += 1
            last_row = r
    print(f"\n5. Amostra do arquivo principal ({os.path.basename(main_csv)}):")
