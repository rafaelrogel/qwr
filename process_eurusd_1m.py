...
# Localiza arquivo archive (14).zip ou archive(14).zip
for f in os.listdir(desktop):
    if "archive" in f.lower() and "14" in f and f.endswith(".zip"):
        target_zip = os.path.join(desktop, f)
        break

if not target_zip:
    print("Arquivo archive (14).zip não encontrado. Lista de zips no Desktop:")
    for f in os.listdir(desktop):
        if f.endswith(".zip"):
            print(" ", f)
    exit(1)
...
dest_copy = os.path.join(scratch_dir, "archive(14).zip")
...
with zipfile.ZipFile(dest_copy, "r") as z:
    for item in z.infolist():
        ...
        if target_name == "README.md":
            target_name = "README_eurusd_1m.md"
