...
# Localiza arquivo archive (12).zip ou archive(12).zip
for f in os.listdir(desktop):
    if "archive" in f.lower() and "12" in f and f.endswith(".zip"):
        target_zip = os.path.join(desktop, f)
        break

if not target_zip:
    print("Arquivo archive(12).zip não encontrado. Lista de zips no Desktop:")
    for f in os.listdir(desktop):
        if f.endswith(".zip"):
            print(" ", f)
    exit(1)
...
dest_copy = os.path.join(scratch_dir, "archive(12).zip")
...
print(f"\n4. Arquivos extraídos: {len(extracted_files)}")
for name, size in extracted_files:
    print(f"   - {name} ({size / (1024*1024):.2f} MB)")

# Amostra do CSV de 1m
main_csv = None
...
if main_csv and os.path.exists(main_csv):
    ...
    print(f"   Total de velas de 1 minuto: {count:,}")
