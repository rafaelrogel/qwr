...
# Localiza arquivo archive (16).zip ou archive(16).zip
for f in os.listdir(desktop):
    if "archive" in f.lower() and "16" in f and f.endswith(".zip"):
        target_zip = os.path.join(desktop, f)
        break

if not target_zip:
    print("Arquivo archive(16).zip não encontrado. Lista de zips no Desktop:")
    for f in os.listdir(desktop):
        if f.endswith(".zip"):
            print(" ", f)
    exit(1)
...
dest_copy = os.path.join(scratch_dir, "archive(16).zip")
