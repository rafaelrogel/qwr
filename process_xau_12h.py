...
# Localiza arquivo archive (18).zip ou archive(18).zip
for f in os.listdir(desktop):
    if "archive" in f.lower() and "18" in f and f.endswith(".zip"):
        target_zip = os.path.join(desktop, f)
        break

if not target_zip:
    print("Arquivo archive(18).zip não encontrado. Lista de zips no Desktop:")
    for f in os.listdir(desktop):
        if f.endswith(".zip"):
            print(" ", f)
    exit(1)

print(f"1. Arquivo localizado: {target_zip} ({os.path.getsize(target_zip) / 1024:.2f} KB)")

scratch_dir = r"C:\Users\rafae\.gemini\antigravity\brain\c125241d-a48c-4ba7-aab4-bba800620ab2\scratch"
os.makedirs(scratch_dir, exist_ok=True)
dest_copy = os.path.join(scratch_dir, "archive(18).zip")
...
with zipfile.ZipFile(dest_copy, "r") as z:
    for item in z.infolist():
        ...
        if target_name == "README.md":
            target_name = "README_xauusd_12h.md"
...
if main_csv and os.path.exists(main_csv):
    ...
    print(f"   Total de velas de 12 horas: {count:,}")
